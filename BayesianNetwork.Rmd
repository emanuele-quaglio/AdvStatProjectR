...
create_bn_dag <- function(net) {

    net_nodes <- names(net$dag)
...
```{r}
saveRDS(bn_mcmc_result, file=paste0('mcmc_ztbin',as.numeric(Sys.time()) * 1000, '.rds'))
...
max_score_vs_iter<-function(chain_scores){ 
  plot(1:length(chain_scores),cummax(chain_scores),xlab='Iteration',ylab='Max(Score)',col='blue',type='l') 
}
```
...
```{r}
bn_ordering_sampling<-function(score_function, data_df, n_iter){
  #score function is a function like k2_algorithm, that given an ordering and data, gives as output the list(network,score) of the corresponding best bayesian network
  nodes<-names(data_df)
  chain<-vector('list', n_iter)
  for(i in 1:n_iter){
...
n_iter<-100
start.time<-proc.time()
bn_ordering_sampling_result<-bn_ordering_sampling(k2_algorithm, sachs, n_iter)
end.time<-proc.time()
cat('bn_ordering_sampling duration' ,n_iter, 'iterations: ', end.time-start.time)
```

```{r}
saveRDS(bn_ordering_sampling_result, file=paste0('unif_sample',as.numeric(Sys.time()) * 1000, '.rds'))
object.size(bn_mcmc_result[['chain']])
network_dag<-bn_ordering_sampling_result[['best']][['dag']]
chain_scores<-sapply(bn_ordering_sampling_result[['chain']], function(el)(el[['score']]))
chain_nodes_orderings<-lapply(bn_ordering_sampling_result[['chain']], function(el)(el[['nodes']]))
print(bn_ordering_sampling_result[['best']][['score']])
print_network(network_dag)
```
